...
export default function IngredientCostPage(){
 ...
 <div className="v2-work"><main className="v2-main ingredient-cost-main">
   ...
   <div className="ingredient-filter-panel">
    ...
    <div className="ingredient-list-meta"><span><b>{visible.length}</b> of {rows.length} ingredients shown{category!=='all'?' · '+category:''}</span><div>{selectedIds.length>0&&<><div className="bulk-category-control"><input list="bulk-category-options" value={bulkCategory} onChange={e=>setBulkCategory(e.target.value)} placeholder="Assign category…"/><datalist id="bulk-category-options">{categories.map(cat=><option key={cat} value={cat}/>)}</datalist><button disabled={saving||!bulkCategory.trim()} onClick={applyBulkCategory}>{saving?'Updating…':'Apply to '+selectedIds.length}</button></div><button className="bulk-delete" onClick={()=>requestDelete(selectedRows)}>Delete selected ({selectedIds.length})</button></>}</div></div>
   </div>
