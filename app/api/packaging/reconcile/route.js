...
function costFields(x){
 ...
 const rawFactor=Number(x.purchaseConversionFactor??m.purchaseConversionFactor??m.sourcePurchaseConversionFactor??NaN);
 const storedBase=Number(x.storedBaseQuantity??m.storedBaseQuantity??NaN);
 const purchaseToStorageFactor=Number.isFinite(rawFactor)&&rawFactor>0?rawFactor:(Number.isFinite(storedBase)&&storedBase>0&&effectiveQty>0?storedBase/effectiveQty:(norm(purchaseUnit)===norm(storageUnit)?1:null));
 return{effectiveQty,purchaseUnit,effectivePrice,storageUnit,storageCost,purchaseToStorageFactor};
}
...
export async function POST(req){
 try{
  ...
  if(b.action==="manual_map"){
    ...
    const name=itemName(x)||local.name,{effectiveQty,purchaseUnit,effectivePrice,storageUnit,storageCost,purchaseToStorageFactor}=costFields(x);
    ...
    await sql`UPDATE packaging_items SET purchase_quantity=${effectiveQty},purchase_unit=${purchaseUnit},purchase_price=${effectivePrice},
      storage_unit=${storageUnit},storage_unit_cost=${storageCost},purchase_to_storage_factor=${purchaseToStorageFactor},unit_cost=${derived.unitCost},costing_unit='each',costing_status=${derived.status},
  ...
  if(b.action==="auto_sync"){
    ...
    const local=await sql`
      SELECT id,name,source_type,integration_id,external_item_id,COALESCE(storage_to_costing_factor,units_per_storage_unit) AS units_per_storage_unit
    ...
    for(const p of local){
      ...
      const extId=externalId(x),name=itemName(x)||p.name,{effectiveQty,purchaseUnit,effectivePrice,storageUnit,storageCost,purchaseToStorageFactor}=costFields(x);
  ...
  for(const id of ids){
    ...
    const {effectiveQty,purchaseUnit,effectivePrice,storageUnit,storageCost,purchaseToStorageFactor}=costFields(x);
    ...
    if(existing){
      ...
      await sql`INSERT INTO packaging_items(tenant_id,name,purchase_quantity,purchase_unit,purchase_price,storage_unit,storage_unit_cost,purchase_to_storage_factor,storage_to_costing_factor,units_per_storage_unit,costing_unit,costing_status,unit_cost,source_type,integration_id,external_item_id,source_metadata,last_source_sync_at)
        VALUES(${tenant.id},${name},${effectiveQty},${purchaseUnit},${effectivePrice},${storageUnit},${storageCost},${purchaseToStorageFactor},NULL,NULL,'each',${derived.status},${derived.unitCost},'shelfsense',${integration.id},${id},${JSON.stringify(x)}::jsonb,NOW())`;
      added++;
    }
