...
export async function POST(req){
  const rid=requestId(req);try{
    ...
    if(action==='create'){
      ...
      const label=`sync-${new Date().toISOString().replace(/[-:.TZ]/g,'').slice(0,14)}`,snapshotId=randomUUID();
      const queries=[sql`INSERT INTO costing_snapshots(id,tenant_id,snapshot_date,label,status) VALUES(${snapshotId},${tenant.id},${asOf}::date,${label},'draft')`];
      for(const r of ingredients){
        const cost=safeNum(r.kitchenCost)??0,source={...r,changed:Boolean(r.changed),needsYield:Boolean(r.needsYield)};
        queries.push(sql`INSERT INTO costing_snapshot_lines(snapshot_id,entity_type,entity_id,entity_name,cost,source_breakdown) VALUES(${snapshotId},'ingredient',${String(r.ingredientId)},${String(r.ingredientName||'Ingredient')},${cost},${JSON.stringify(source)}::jsonb)`);
      }
      for(const m of menus){
        ...
        queries.push(sql`INSERT INTO costing_snapshot_lines(snapshot_id,entity_type,entity_id,entity_name,cost,selling_price,food_cost_percent,contribution,source_breakdown) VALUES(${snapshotId},'menu',${String(m.recipeId)},${String(m.name||'Menu item')},${cost},${sell},${food},${contribution},${JSON.stringify(m)}::jsonb)`);
      }
    ...
    if(action==='publish'){
      ...
      const queries=[sql`UPDATE costing_snapshots SET status='archived' WHERE tenant_id=${tenant.id} AND status='published' AND id<>${snapshot.id}`];
      let inserted=0;
      for(const l of ingredientLines){
        ...
        if(!m.sourceExternalId&&!m.changed)continue;
        queries.push(sql`INSERT INTO ingredient_prices(tenant_id,ingredient_id,purchase_quantity,purchase_unit,purchase_price,supplier,price_date,source,source_external_id,source_metadata)
          SELECT ${tenant.id},${l.entity_id}::uuid,${Number(m.purchaseQuantity)},${m.purchaseUnit},${Number(m.purchasePrice)},${m.supplier||'ShelfSense'},${m.priceDate||snapshot.snapshot_date}::date,'shelfsense',${m.sourceExternalId||null},${JSON.stringify({...m,publishedSnapshotId:snapshot.id,sourceBaseUnit:m.storageUnit||m.sourceBaseUnit,sourceUnitCost:m.storageUnitCost??m.sourceUnitCost})}::jsonb
          WHERE ${m.sourceExternalId||null}::text IS NULL OR NOT EXISTS(SELECT 1 FROM ingredient_prices WHERE tenant_id=${tenant.id} AND ingredient_id=${l.entity_id}::uuid AND source='shelfsense' AND source_external_id=${m.sourceExternalId||null})`);
        inserted++;
      }
    ...
    if(action==='restore'){
      ...
      for(const l of ingredientLines){
        const m=meta(l.source_breakdown);
        if(!(Number(m.purchaseQuantity)>0)||!Number.isFinite(Number(m.purchasePrice))||!m.purchaseUnit)continue;
        queries.push(sql`INSERT INTO ingredient_prices(tenant_id,ingredient_id,purchase_quantity,purchase_unit,purchase_price,supplier,price_date,source,source_external_id,source_metadata) VALUES(${tenant.id},${l.entity_id}::uuid,${Number(m.purchaseQuantity)},${m.purchaseUnit},${Number(m.purchasePrice)},${m.supplier||'Snapshot restore'},CURRENT_DATE,'restore',NULL,${JSON.stringify({...m,restoredFromSnapshotId:snapshot.id,originalPriceDate:m.priceDate||null,sourceBaseUnit:m.storageUnit||m.sourceBaseUnit,sourceUnitCost:m.storageUnitCost??m.sourceUnitCost})}::jsonb)`);
        if(m.yieldSourceUnit&&Number(m.yieldUsableQuantity)>0&&m.yieldCostingUnit)queries.push(sql`INSERT INTO ingredient_costing_conversions(tenant_id,ingredient_id,purchase_unit,usable_quantity,costing_unit,source,notes,updated_at) VALUES(${tenant.id},${l.entity_id}::uuid,${m.yieldSourceUnit},${Number(m.yieldUsableQuantity)},${m.yieldCostingUnit},'manual','Restored from costing snapshot',NOW()) ON CONFLICT (tenant_id,ingredient_id,purchase_unit) DO UPDATE SET usable_quantity=EXCLUDED.usable_quantity,costing_unit=EXCLUDED.costing_unit,notes=EXCLUDED.notes,updated_at=NOW()`);
        restored++;
      }
      queries.push(sql`UPDATE costing_snapshots SET status='published' WHERE id=${snapshot.id} AND tenant_id=${tenant.id}`);
      await sql.transaction(queries,{isolationLevel:"Serializable"});
