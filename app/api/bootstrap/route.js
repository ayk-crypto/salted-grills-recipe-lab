...
export async function GET() {
  try {
    ...
    const categories = await sql`SELECT * FROM categories WHERE is_active = true ORDER BY name`;
    ...
    const prices = await sql`
      SELECT ip.id,ip.ingredient_id,ip.purchase_quantity,ip.purchase_unit,ip.purchase_price,ip.price_date,ip.supplier,i.name AS ingredient_name
      FROM ingredient_prices ip JOIN ingredients i ON i.id=ip.ingredient_id
      WHERE i.is_active=true ORDER BY ip.price_date DESC,ip.created_at DESC LIMIT 1000
    `;
    return NextResponse.json({ingredients, packaging, units, categories, recipes, prices});
  } catch (e) {
